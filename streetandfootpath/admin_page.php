<?php

@include 'config.php';

session_start();

$admin_id = $_SESSION['admin_id'];

if (!isset($admin_id)) {
   header('location:login.php');
}

?>

<!DOCTYPE html>
<html lang="en">

<head>
   <meta charset="UTF-8">
   <meta http-equiv="X-UA-Compatible" content="IE=edge">
   <meta name="viewport" content="width=device-width, initial-scale=1.0">
   <title>admin page</title>

   <!-- font awesome cdn link  -->
   <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css">

   <!-- custom css file link  -->
   <link rel="stylesheet" href="css/adminstyle.css">

</head>

<body>

   <?php include 'admin_header.php'; ?>

   <section class="dashboard">


      
      <div class="box-container">
         <div class="box">
            <?php
            $select_orders = $conn->prepare("SELECT * FROM `orders`");
            $select_orders->execute();
            $number_of_orders = $select_orders->rowCount();
            ?>
            <h3><?= $number_of_orders; ?></h3>
            <p>ปัญหาทั้งหมด</p>
            <a href="admin_orders.php" class="btn">see problems</a>
         </div>

         <div class="box">
            <?php
            $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE method = ?");
            $select_orders->execute(['street']);
            $number_of_orders = $select_orders->rowCount();
            ?>
            <h3><?= $number_of_orders; ?></h3>
            <p>ปัญหาท้องถนน</p>
            <a href="admin_category.php?category=street" class="btn">see problems</a>
         </div>

         <div class="box">
            <?php
            $select_orders = $conn->prepare("SELECT * FROM `orders` WHERE method = ?");
            $select_orders->execute(['footpath']);
            $number_of_orders = $select_orders->rowCount();
            ?>
            <h3><?= $number_of_orders; ?></h3>
            <p>ปัญหาทางเท้า</p>
            <a href="admin_category.php?category=footpath" class="btn">see problems</a>
         </div>

         <div class="box">
            <?php
            $select_pendings = $conn->prepare("SELECT * FROM `orders` WHERE status = ?");
            $select_pendings->execute(['pending']);
            $number_of_pendings = $select_pendings->rowCount();
            ?>
            <h3><?= $number_of_pendings; ?></h3>
            <p>ดำเนินการ</p>
            <a href="admin_orders.php" class="btn">see problems</a>
         </div>

         <div class="box">
            <?php
            $select_completed = $conn->prepare("SELECT * FROM `orders` WHERE status = ?");
            $select_completed->execute(['completed']);
            $number_of_completed = $select_completed->rowCount();
            ?>
            <h3><?= $number_of_completed; ?></h3>
            <p>เสร็จสิ้น</p>
            <a href="admin_orders.php" class="btn">see problems</a>
         </div>
         <div class="box">
            <?php
            $select_accounts = $conn->prepare("SELECT * FROM `users`");
            $select_accounts->execute();
            $number_of_accounts = $select_accounts->rowCount();
            ?>
            <h3><?= $number_of_accounts; ?></h3>
            <p>จำนวนบัญชีทั้งหมด</p>

         </div>
         <div class="box">
            <?php
            $select_users = $conn->prepare("SELECT * FROM `users` WHERE user_type = ?");
            $select_users->execute(['user']);
            $number_of_users = $select_users->rowCount();
            ?>
            <h3><?= $number_of_users; ?></h3>
            <p>จำนวนบัญชีผู้ใช้</p>
         </div>

         <div class="box">
            <?php
            $select_admins = $conn->prepare("SELECT * FROM `users` WHERE user_type = ?");
            $select_admins->execute(['admin']);
            $number_of_admins = $select_admins->rowCount();
            ?>
            <h3><?= $number_of_admins; ?></h3>
            <p>จำนวนบัญชีแอดมิน</p>

         </div>




      </div>

   </section>













   <script src="js/script.js"></script>

</body>

</html>